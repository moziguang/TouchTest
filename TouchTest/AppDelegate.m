...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    self.window = [[EyuWindow alloc] initWithFrame: [[UIScreen mainScreen] bounds]];
    
    // Use RootViewController to manage CCEAGLView
    //    NSLog(@"lwq, initWithNibName 11111 thread = %@", [NSThread currentThread]);
    ViewController* viewController = [[ViewController alloc] initWithNibName:@"View" bundle:nil];
    //    NSLog(@"lwq, initWithNibName 22222");
...
- (NSPersistentContainer *)persistentContainer {
    ...
    @synchronized (self) {
        if (_persistentContainer == nil) {
            _persistentContainer = [[NSPersistentContainer alloc] initWithName:@"TouchTest"];
